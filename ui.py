...
initConvId = "5e409dbc43e14f86a40cf432fe8ee76d"
url = "http://localhost:8000/"
# Set Streamlit page configuration
st.set_page_config(page_title="🧠My Personal Bot🤖", layout="wide")
...
if "conversation_id" not in st.session_state:
    st.session_state["conversation_id"] = initConvId
...
def get_history():
    ...
    payload = {"conversation_id": st.session_state.conversation_id, "query": ""}
    response = requests.post(url+"history", json=payload).json()
    print(response)
    return response

# st.session_state["stored_session"].append(get_history())
...
def send_text():
    ...
    if user_input:
        ...
        response = requests.post(url+"converse", json=payload).json()
...
st.subheader(" ConvId: "+st.session_state["conversation_id"])
